...
class Command(BaseCommand):
    ...
    def create_jobs_and_phases(self, count):
        ...
        for i in range(count):
            ...
            job_start_date = today + timedelta(days=random.randint(start_low, start_high))
        ...
        for phase in phases:
            ...
            if phase.desired_delivery_date < timezone.now().date() - timedelta(days=7):
                ...
                status_choice = random.choice([
                    PhaseStatuses.DRAFT, PhaseStatuses.PENDING_SCHED, PhaseStatuses.PENDING_SCHED,
                    PhaseStatuses.SCHEDULED_TENTATIVE, PhaseStatuses.SCHEDULED_TENTATIVE,
                    PhaseStatuses.SCHEDULED_CONFIRMED, PhaseStatuses.SCHEDULED_CONFIRMED,
                    PhaseStatuses.CLIENT_NOT_READY, PhaseStatuses.READY_TO_BEGIN, PhaseStatuses.PRE_CHECKS,
                ])
        ...
        if job_phases:
            ...
            if max_phase_status >= PhaseStatuses.COMPLETED:
                ...
                job.status = random.choice([
                    JobStatuses.DRAFT, JobStatuses.PENDING_SCOPE, JobStatuses.SCOPING,
                    JobStatuses.SCOPING, JobStatuses.SCOPING_ADDITIONAL_INFO_REQUIRED,
                    JobStatuses.PENDING_SCOPING_SIGNOFF,
                ])
            # A slice of early-stage opportunities are lost, for a realistic pipeline.
            if job.status in (JobStatuses.DRAFT, JobStatuses.SCOPING) and random.random() < 0.15:
    ...
    def progress_phase_to_completion(self, phase):
        ...
        phase.status = random.choice([
            PhaseStatuses.DELIVERED, PhaseStatuses.DELIVERED, PhaseStatuses.DELIVERED,
