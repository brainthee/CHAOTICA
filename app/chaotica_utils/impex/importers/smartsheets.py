    ...
    def import_data(self, data):
        ...
        for project_file in data:
            ...
            for row in reader:
                ...
                if projectData == None:
                    if (
                        row["Client"].strip() is not None
                        and row["Client"].strip() is not ""
                    ):
                ...
                if row["Phase Name"] == "Summary":
                    ...
                    if phaseState:
                        if (
                            phaseState
                            not in projectData["phases"][row["Phase Name"]]["state"]
                        ):
                            projectData["phases"][row["Phase Name"]]["state"][
                                phaseState
                            ] = []
                        ...
                        assigned = [
                            x.strip() for x in re.split(";|,", row["Assigned To"])
                        ]
            ...
            db_lead = self._convert_name_to_db_user(
                projectData["lead"]
            ...
            for am in ams:
                db_user_am, db_user_am_created = User.objects.get_or_create(
                    email__iexact=am, defaults={"email": am}
                )
                if not primary_am:
