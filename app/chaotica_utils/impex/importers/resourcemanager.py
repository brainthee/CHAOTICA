...
class ResourceManagerUserImporter(BaseImporter):
    ...
    def _convert_name_to_db_user(self, user):
        ...
        if "@" in user:
            ...
            db_user, _ = User.objects.get_or_create(
                email__iexact=user, defaults={"email": user}
            )
        else:
            # It's likely a name...
            user = user.split(" ")
            if len(user) == 2:
                ...
                if User.objects.filter(first_name=user[0], last_name=user[1]).exists():
                    db_user = User.objects.filter(
                        ...
                        0
                    ]  # Yes yes, might be more than one 'John Doe' but yolo :D
        return db_user
    ...
    def import_data(self, data):
        ...
        for user_file in data:
            ...
            db_user, _ = User.objects.get_or_create(
                email__iexact=user_data["email"], defaults={"email": user_data["email"]}
            )
            ...
            for custom_field in user_data["data"]:
                if custom_field["custom_field_name"] == "Market Unit":
                    ...
                    if org_name:
                        if not OrganisationalUnitMember.objects.filter(
                            member=db_user, unit__name=custom_field["value"]
                        ).exists():
                            # Add them!
                        ...
                        if len(skill) > 1:
                            ...
                            if SkillCategory.objects.filter(name=skill[0]).exists():
                                ...
                                if Skill.objects.filter(
                                    name=skill[1], category=db_skill_cat
                                ).exists():
                                    db_skill = Skill.objects.get(
                                        name=skill[1], category=db_skill_cat
                                    )
                                    if not UserSkill.objects.filter(
                                        user=db_user, skill=db_skill
                                    ).exists():
                                        # Add the skill!
                                        UserSkill.objects.create(
                                            user=db_user, skill=db_skill, rating=rating
                                        )
            db_user.save()
