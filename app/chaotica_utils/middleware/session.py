...
class SessionMiddleware(DjSessionMiddleware):
    def __init__(self, get_response):
        ...
        for proxy in self.trusted_proxies:
            try:
                self.compiled_trusted_proxies.append(
        ...
        for host in self.allowed_hosts:
            if host == "*":
                ...
                self.allowed_host_patterns.append(
                    re.compile(f"^(.+{pattern}|{pattern[2:]})$", re.IGNORECASE)
                )
            elif "*" in host:
                ...
                # Exact match
                self.allowed_host_patterns.append(
                    re.compile(f"^{re.escape(host)}$", re.IGNORECASE)
                )
    ...
    def process_request(self, request):
        ...
        if not self.validate_host(request):
            ...
            return HttpResponseBadRequest(
                "Invalid HTTP_HOST header", content_type="text/plain"
            )
