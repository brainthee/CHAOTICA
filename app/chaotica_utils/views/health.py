...
class HealthCheckView(View):
    def get(self, request):
        ...
        try:
            ...
            # Find the API key
            api_key_obj = (
                HealthCheckAPIKey.objects.filter(key=key_uuid, is_active=True)
                .select_related("user")
                .first()
            )
        ...
        try:
            ...
            if cache_value == "test":
                health_status["checks"]["cache"] = {"status": "ok", "backend": "redis"}
            else:
            ...
            if settings.EMAIL_BACKEND == "django.core.mail.backends.smtp.EmailBackend":
                ...
                try:
                    ...
                    result = sock.connect_ex(
                        (settings.EMAIL_HOST, int(settings.EMAIL_PORT))
                    )
                    sock.close()
                    ...
                    health_status["checks"]["email"]["connectivity_error"] = str(
                        conn_error
                    )
                    health_status["checks"]["email"]["status"] = "warning"
        ...
        try:
            ...
            if rm_api_site and rm_api_token:
                ...
                try:
                    ...
                    health_status["checks"]["rm_sync"]["connectivity_error"] = str(
                        req_error
                    )
                    health_status["checks"]["rm_sync"]["status"] = "warning"

        except Exception as e:
            health_status["checks"]["rm_sync"] = {"status": "error", "error": str(e)}
        ...
        try:
            ...
            health_status["checks"]["storage"] = {"status": "error", "error": str(e)}
            health_status["status"] = "unhealthy"
        ...
        try:
            ...
            health_status["checks"]["clamav"] = {"status": "error", "error": str(e)}
        ...
        try:
            ...
            if adfs_enabled:
                ...
                health_status["checks"]["sso_adfs"]["tenant_id"] = (
                    tenant_id if tenant_id != "xx" else "not_configured"
                )
                health_status["checks"]["sso_adfs"]["client_id"] = (
                    client_id if client_id != "xx" else "not_configured"
                )
                health_status["checks"]["sso_adfs"]["has_secret"] = bool(
                    client_secret and client_secret != "xx"
                )
                health_status["checks"]["sso_adfs"][
                    "auto_login"
                ] = config.ADFS_AUTO_LOGIN
                ...
                if tenant_id == "xx" or client_id == "xx" or client_secret == "xx":
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            try:
                                ...
                                token_response = requests.post(
                                    token_url, data=token_data, timeout=5
                                )
                        ...
                        health_status["checks"]["sso_adfs"]["connectivity_error"] = str(
                            req_error
                        )
                        health_status["checks"]["sso_adfs"]["status"] = "warning"

        except Exception as e:
            health_status["checks"]["sso_adfs"] = {"status": "error", "error": str(e)}
        ...
        try:
            ...
            health_status["metrics"] = {
                "users": {
                    ...
                    "with_roles": User.objects.filter(
                        is_active=True, groups__isnull=False
                    ).count(),
                },
                "sessions": {
                    "total": Session.objects.count(),
                    "active": Session.objects.filter(expire_date__gt=timezone.now())
            ...
            health_status["metrics"] = {"status": "error", "error": str(e)}
        ...
        try:
            ...
            for cron_class_path in configured_jobs:
                ...
                latest_log = (
                    CronJobLog.objects.filter(code=job_code)
                    .order_by("-start_time")
                    .first()
                )
                ...
                if latest_log:
                    job_info = {
                        "last_run": (
                            latest_log.start_time.isoformat()
                        ...
                        "run_time_seconds": (
                            latest_log.run_time
                            if hasattr(latest_log, "run_time")
                            else None
                        ),
                    }
                    ...
                    if latest_log.start_time:
                        ...
                        try:
                            ...
                            if hasattr(cron_class, "RUN_EVERY_MINS"):
                                expected_interval = timedelta(
                                    minutes=cron_class.RUN_EVERY_MINS
                                )
                                job_info["expected_interval_minutes"] = (
                                    cron_class.RUN_EVERY_MINS
                                )
                                job_info["overdue"] = time_since_run > (
                                    expected_interval * 2
                                )  # Allow 2x the interval before marking overdue
                            # Check for RUN_AT_TIMES
                                ...
                                job_info["overdue"] = time_since_run > timedelta(
                                    hours=25
                                )
                            else:
                        ...
                        job_info["hours_since_last_run"] = round(
                            time_since_run.total_seconds() / 3600, 2
                        )
                    ...
                    job_info = {"status": "never_run", "last_run": None}

                cron_status[job_code] = job_info

            # Overall cron health
            has_failures = any(
                job.get("status") == "warning" for job in cron_status.values()
            )
            never_run = any(
                job.get("status") == "never_run" for job in cron_status.values()
            )
            ...
            health_status["checks"]["cron_jobs"] = {"status": "error", "error": str(e)}

        # Set overall status
        if any(
            check.get("status") == "error" for check in health_status["checks"].values()
        ):
            health_status["status"] = "unhealthy"
        elif any(
        ...
        return JsonResponse(
            health_status, status=status_code, json_dumps_params={"indent": 2}
        )
