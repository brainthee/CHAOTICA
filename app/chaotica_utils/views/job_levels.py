...
from ..forms.job_levels import (
    JobLevelForm,
    AssignJobLevelForm,
    ImportJobLevelAssignmentsForm,
)
...
def job_level_list(request):
    ...
    job_levels = JobLevel.objects.annotate(
        current_user_count=Count(
            "user_assignments", filter=Q(user_assignments__is_current=True)
        )
    ).order_by("order")
...
def job_level_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            job_level = form.save()
            messages.success(
                request, f"Job level '{job_level.short_label}' created successfully"
            )
...
def job_level_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            job_level = form.save()
            messages.success(
                request, f"Job level '{job_level.short_label}' updated successfully"
            )
            return redirect("job_level_list")
    else:
    ...
    current_assignment_count = UserJobLevel.objects.filter(
        job_level=job_level, is_current=True
    ).count()
...
def job_level_delete(request, pk):
    ...
    if active_assignments > 0:
        messages.error(
            request,
            f"Cannot delete job level '{job_level.short_label}' - {active_assignments} users are currently assigned to it",
        )
        return redirect("job_level_list")
    ...
    try:
        ...
        job_level.save()
        messages.success(
            request, f"Job level '{job_level.short_label}' has been deactivated"
        )
...
def user_job_level_history(request):
    ...
    if search_email:
        assignments = assignments.filter(user__email__icontains=search_email)
        ...
        if form.is_valid():
            ...
            try:
                ...
                with transaction.atomic():
                    for row_num, row in enumerate(
                        ...
                        try:
                            ...
                            job_level_short = row.get(
                                "job_level_short_label", ""
                            ).strip()
                            ...
                            if (
                                not email
                                or not job_level_short
                                or not assigned_date_str
                            ):
                            ...
                            try:
                                ...
                                errors.append(
                                    f"Row {row_num}: User with email '{email}' not found"
                                )
                                error_count += 1
                                continue
                            ...
                            # Get job level
                            try:
                                job_level = JobLevel.objects.get(
                                    short_label=job_level_short, is_active=True
                                )
                            except JobLevel.DoesNotExist:
                                errors.append(
                                    f"Row {row_num}: Job level '{job_level_short}' not found or inactive"
                                )
                                error_count += 1
                                continue
                            ...
                            try:
                                assigned_date = datetime.strptime(
                                    assigned_date_str, "%Y-%m-%d"
                                ).date()

                                # Validate date is not in future
                                if assigned_date > timezone.now().date():
                                    errors.append(
                                        f"Row {row_num}: Assignment date cannot be in the future"
                                    )
                                    error_count += 1
                                    continue
                            except ValueError:
                                errors.append(
                                    f"Row {row_num}: Invalid date format '{assigned_date_str}' (use YYYY-MM-DD)"
                                )
                                error_count += 1
                                continue
                            ...
                            if current_level and not update_existing:
                                errors.append(
                                    f"Row {row_num}: User {email} already has a job level assigned (skipped)"
                                )
                                continue
                            ...
                            if (
                                not current_level
                                or current_level.job_level != job_level
                            ):
                ...
                if success_count > 0:
                    messages.success(
                        request,
                        f"Successfully imported {success_count} job level assignment(s)",
                    )
...
def download_job_level_template(request):
    ...
    response["Content-Disposition"] = (
        'attachment; filename="job_level_assignments_template.csv"'
    )
