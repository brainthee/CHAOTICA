...
class SetupWizardView(View):
    ...
    def get_context(self, request, step):
        ...
        context = {
            ...
            "progress_percent": (self.steps.index(step) / max(1, len(self.steps) - 1))
            * 100,
        }
    ...
    def handle_admin_user(self, request):
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Admin user '{user.email}' created successfully!"
            )
            return redirect(f"{request.path}?step=organisation")
    ...
    def handle_organisation(self, request):
        ...
        if form.is_valid():
            ...
            manager_role, role_created = OrganisationalUnitRole.objects.get_or_create(
                pk=UnitRoles.MANAGER, defaults={"name": "Manager", "manage_role": True}
            )
            if role_created:
            ...
            messages.success(
                request, f"Organisation unit '{org_unit.name}' created successfully!"
            )
    ...
    def handle_skills(self, request):
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Created {len(category_objects)} categories and {len(created_skills)} skills!",
            )
    ...
    def has_existing_data(self):
        ...
        return (
            ...
            or OrganisationalUnit.objects.count() > 0
            or Service.objects.count() > 0
            or SkillCategory.objects.count() > 0
            or Client.objects.count() > 0
        )
