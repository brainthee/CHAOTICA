...
def download_db_backup(request):
    ...
    if start_new:
        ...
        job = ManualBackupJob.objects.create(
            user=request.user, backup_type="db", status="pending"
        )
    ...
    if not job_id:
        return JsonResponse(
            {"status": "no_job", "message": "No backup job in progress."}
        )
    ...
    if job.status == "pending":
        # Still in queue
        position = (
            ManualBackupJob.objects.filter(
                status="pending", created_at__lt=job.created_at
            ).count()
            + 1
        )
        ...
        if request.GET.get("download", "false").lower() == "true":
            try:
                ...
                response = FileResponse(
                    open(job.file_path, "rb"), content_type=job.get_content_type()
                )
                response["Content-Disposition"] = (
                    f'attachment; filename="{job.get_filename()}"'
                )
        ...
        time_taken = (
            (job.completed_at - job.started_at).seconds
    ...
    return JsonResponse({"status": "unknown", "message": "Unknown job status"})
...
def download_media_backup(request):
    ...
    if start_new:
        # Explicitly requested new backup - create it
        job = ManualBackupJob.objects.create(
            user=request.user, backup_type="media", status="pending"
        )
    ...
    if job.status == "pending":
        ...
        if request.GET.get("download", "false").lower() == "true":
            try:
                ...
                # Serve the file
                response = FileResponse(
                    open(job.file_path, "rb"), content_type="application/gzip"
                )
                response["Content-Disposition"] = (
                    'attachment; filename="media_backup.tar.gz"'
                )
...
def restore_db_backup(request):
    ...
    is_confirmation_step = (
        request.POST.get("db_confirmed_restore", "false").lower() == "true"
    )
    ...
    if not form.is_valid():
        return JsonResponse(
            {"status": "error", "errors": form.errors, "message": "Validation failed"},
            status=400,
        )
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": f"Restore failed: {str(e)}"}, status=400
        )
...
def restore_media_backup(request):
    ...
    # Check if this is the confirmation step
    is_confirmation_step = (
        request.POST.get("media_confirmed_restore", "false").lower() == "true"
    )
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": f"Media restore failed: {str(e)}"},
            status=400,
        )
