...
class Qualification(models.Model):
    awarding_body = models.ForeignKey(AwardingBody, related_name='qualifications', on_delete=models.PROTECT)    
    name = models.CharField(max_length=200)
    ...
    validity_period = models.DurationField('Validity Period', 
        help_text="How many days the qualification is valid for. If left empty, it does not expire.", 
        null=True, blank=True)

    url = models.URLField(max_length=2000, default="", null=True, blank=True, verbose_name="Qualification URL")
    guidance_url = models.URLField(max_length=2000, default="", null=True, blank=True, verbose_name="Guidance URL")
    ...
    def get_absolute_url(self):
        return reverse("qualification_detail", kwargs={"bodySlug": self.awarding_body.slug, "slug": self.slug})
...
class QualificationRecord(models.Model):
    qualification = models.ForeignKey(Qualification, related_name='users', on_delete=models.PROTECT)
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="qualifications",
    )
    status = models.IntegerField(choices=QualificationStatus.CHOICES, default=QualificationStatus.UNKNOWN)
    history = HistoricalRecords()

    last_updated_on = models.DateTimeField(auto_now=True)
    status_changed_date = MonitorField(monitor='status')
    attempt_date = models.DateField('Date of Attempt', null=True, blank=True, 
    ...
    lapse_date = models.DateField('Date Lapses', null=True, blank=True, 
    ...
    def __str__(self):
        return u'%s - %s (%s)' % (self.user,
                                    self.qualification, self.get_status_display())