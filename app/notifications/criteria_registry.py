...
def timeslot_assignees(entity, params=None):
    ...
    if hasattr(entity, 'timeslots'):
        # This is a phase
        return User.objects.filter(
            timeslots__phase=entity,
            is_active=True
        ).distinct()
    elif hasattr(entity, 'phases'):
        # This is a job
        return User.objects.filter(
            timeslots__phase__job=entity,
            is_active=True
        ).distinct()
...
def user_manager(entity, params=None):
    ...
    if hasattr(entity, 'user') and entity.user.manager:
        managers = [entity.user.manager]
        if entity.user.acting_manager:
            managers.append(entity.user.acting_manager)
        return User.objects.filter(id__in=[m.id for m in managers if m], is_active=True)
...
def job_support_team(entity, params=None):
    ...
    if job and hasattr(job, 'job_support_roles'):
        support_team_users = [role.user for role in job.job_support_roles.all() if role.user]
        return User.objects.filter(id__in=[user.id for user in support_team_users], is_active=True)
...
def same_client_job_leads(entity, params=None):
    ...
    active_jobs = Job.objects.filter(client=client).exclude(status__in=[4, 5])  # Exclude deleted/archived
